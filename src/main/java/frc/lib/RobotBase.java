...
public abstract class RobotBase implements AutoCloseable {
  ...
  private static <T extends RobotBase> void runRobot(Supplier<T> robotSupplier) {
    PLog.info("RobotBase","********** Robot program starting **********");
    ...
    try {
      ...
      PLog.fatalException("RobotBase", "Unhandled exception instantiating robot " + robotName + " " + throwable, throwable.getStackTrace());
      return;
    }
    ...
    try {
      ...
      PLog.fatalException("RobotBase","Unhandled exception: " + throwable, throwable.getStackTrace());
      errorOnExit = true;
    } finally {
      m_runMutex.lock();
